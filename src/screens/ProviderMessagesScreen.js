...
export default function ProviderMessagesScreen() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      await refetch();
      Alert.alert(t('provider.messages.sent'), t('provider.messages.sentMessage'));
    } catch (error) {
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <ScrollView contentContainerStyle={styles.container}>
        <EBText variant="title">{t('provider.messages.title')}</EBText>
        <EBText variant="body" color="secondary">
          {t('provider.messages.subtitle')}
        </EBText>

        <View style={styles.composeCard}>
          <TextInput
            style={styles.input}
            placeholder={t('provider.messages.subject')}
            value={form.subject}
            onChangeText={(subject) => setForm((prev) => ({ ...prev, subject }))}
          />
          <TextInput
            style={[styles.input, styles.textarea]}
            placeholder={t('provider.messages.body')}
            value={form.body}
